...
import { SearchFilter_SortBy, SearchFilter_Filters_UploadDate, SearchFilter_Filters_SearchType, SearchFilter_Filters_Duration } from '../protos/generated/misc/params.js';
...
class Innertube {
    ...
    async getInfo(target, client) {
        ...
        const player_payload = PlayerEndpoint.build({
            ...
            client: client,
            sts: __classPrivateFieldGet(this, _Innertube_session, "f").player?.sts,
    ...
    async getBasicInfo(video_id, client) {
        ...
        const response = await this.actions.execute(PlayerEndpoint.PATH, PlayerEndpoint.build({
            video_id: video_id,
            client: client,
            sts: __classPrivateFieldGet(this, _Innertube_session, "f").player?.sts,
            po_token: __classPrivateFieldGet(this, _Innertube_session, "f").po_token
    ...
    async search(query, filters = {}) {
        ...
        const response = await this.actions.execute(SearchEndpoint.PATH, SearchEndpoint.build({
    ...
    async getChannelsFeed() {
        const response = await this.actions.execute(BrowseEndpoint.PATH, { ...BrowseEndpoint.build({ browse_id: 'FEchannels' }), parse: true });
        return new Feed(this.actions, response);
    }
    async getChannel(id) {
        throwIfMissing({ id });
    ...
    async getNotifications() {
        const response = await this.actions.execute(GetNotificationMenuEndpoint.PATH, GetNotificationMenuEndpoint.build({
     ...
     */
    async getPlaylists() {
        const response = await this.actions.execute(BrowseEndpoint.PATH, { ...BrowseEndpoint.build({ browse_id: 'FEplaylist_aggregation' }), parse: true });
        return new Feed(this.actions, response);
    }
    ...
    async getHashtag(hashtag) {
        ...
        const params = encodeURIComponent(u8ToBase64(writer.finish()));
        const response = await this.actions.execute(BrowseEndpoint.PATH, BrowseEndpoint.build({
            browse_id: 'FEhashtag',
            params
        }));
    ...
    async resolveURL(url) {
        const response = await this.actions.execute(ResolveURLEndpoint.PATH, { ...ResolveURLEndpoint.build({ url }), parse: true });
